...
mod tests {
    ...
    #[allow(
        ...
        reason = "Ken French publishes its archive/member names with a literal \
                  uppercase `_CSV.zip` / `.CSV` extension; the exact-case suffix \
                  assertion is intentional, not a user-supplied filename check"
    )]
