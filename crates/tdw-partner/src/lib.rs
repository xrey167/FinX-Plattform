...
//! (`finx-partner` §1, partner-system W2).
//!
...
/// A surface-agnostic event in a partner turn (`finx-partner` §1.2).
///
...
/// The input to a partner turn (`finx-partner` §1.2).
...
/// Cross-turn / per-surface context threaded into a turn (`finx-partner` §1.2).
///
...
/// The terminal outcome of a partner turn (`finx-partner` §1.2).
#[derive(Clone, Debug, PartialEq, Eq)]
...
/// The shared conversational front door (`finx-partner` §1.2).
///
...
pub struct PartnerCore {
    ...
    /// answer (reused per `finx-partner` §10 open-question: one credential gate).
...
impl PartnerCore {
    ...
    /// THE single decision point for a partner turn (`finx-partner` §1.3).
    ///
