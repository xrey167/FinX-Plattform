...
//!
//! Loads every committed seed under `tests/corpus/sql_guard/` and feeds it to
//! the `__fuzz_sql_guard` shim, asserting the read-only SQL guard never panics
//! on arbitrary bytes (multi-statement, unsafe tokens, non-SELECT). The same
//! shim is reused by the nightly cargo-fuzz target.
...
#[test]
fn sql_guard_corpus_never_panics() {
    for (path, bytes) in read_corpus("sql_guard") {
        tdw_exec::__fuzz_sql_guard(&bytes);
        let _ = path;
    }
}
