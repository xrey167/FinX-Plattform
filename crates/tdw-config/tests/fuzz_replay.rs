...
//!
//! Loads every committed seed under `tests/corpus/config_toml/` and feeds it to
//! the `__fuzz_config_toml` shim, asserting the parser never panics on
...
#[test]
fn config_toml_corpus_never_panics() {
    for (path, bytes) in read_corpus("config_toml") {
        tdw_config::__fuzz_config_toml(&bytes);
        let _ = path;
    }
}
