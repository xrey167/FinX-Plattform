...
- [x] Dependency direction reviewed: local dependencies are tdw-event; reverse dependencies are tdw-service-api.
...
- Actor context now supports explicit caller-supplied actor/origin/trace identity instead of forcing the sample service helper.
- Added ActorContext::new and a test proving tenant, host, and trace identity are preserved.
- Follow-up boundary: Higher runtime layers must supply real identity-plane values rather than relying on ActorContext::service samples.
