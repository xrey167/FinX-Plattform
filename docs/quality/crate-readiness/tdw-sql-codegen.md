...
- [x] Dependency direction reviewed: local dependencies are tdw-domain; reverse dependencies remain bounded by the matrix inventory.
...
- DDL export stays target-explicit for Postgres and ClickHouse and is generated from checked-in SQL assets.
- Idempotence and target-specific tests confirm repeated exports are stable, Postgres output stays free of MergeTree DDL, and ClickHouse output includes MergeTree ordering.
- Follow-up boundary: richer schema diffing can layer on top of this static export contract.
