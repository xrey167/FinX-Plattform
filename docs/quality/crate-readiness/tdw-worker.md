...
- Binary delegates provider fetch and worker event-spine evidence to `tdw-service-api`.
- It has explicit stderr/non-zero error handling and no local provider or event-spine duplication.
- Scan signal is the worker sample call; no stub, copied FinX-XR, OpenBB, or fallback path was found.
...
Ready with follow-ups. The worker binary is a thin service API entrypoint; durable job scheduling is intentionally outside this bootstrap crate.
