...
- Pipeline jobs now have validate_jobs coverage for empty pipelines, empty args, duplicate names, self dependencies, and unknown dependencies.
- The market-data dbt DAG is deterministic and keeps silver/gold/test ordering explicit.
- Follow-up boundary: scheduling and retries belong in a later runtime/service layer.
