# tdw-storage-router Readiness Worksheet

Owner tranche: G003-data-storage-pipeline-and-sql-crates - Data, Storage, Pipeline, and SQL Crates.

## Baseline Inventory

- Manifest: crates\tdw-storage-router\Cargo.toml
- Target kinds: lib
- Local dependencies: tdw-core
- External dependencies: async-trait ^0.1.89
- Dev dependencies: schemars ^1.2.1; serde ^1.0.228 features=[derive]; tokio ^1.52.3 features=[macros, rt-multi-thread, sync]
...
- StorageRouter now rejects writes with no registered sinks and reports degraded health instead of silently looking healthy.
- Added fan-out evidence proving each registered sink receives writes and aggregate receipts sum row counts.
- Tests cover specialist sink registration and the empty-router failure mode.
- Follow-up boundary: fan-out retry/partial-failure policy is a later production router concern.
