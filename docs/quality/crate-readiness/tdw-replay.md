# tdw-replay Readiness Worksheet

Owner tranche: G002-core-contracts-event-session-and-replay-crates - Core Contracts, Event, Session, and Replay Crates.

## Baseline Inventory

- Manifest: crates\tdw-replay\Cargo.toml
- Target kinds: lib
- Local dependencies: tdw-cdc, tdw-protocol, tdw-rollout
- External dependencies: serde ^1.0.228 features=[derive]; serde_json ^1.0.145 kind=dev
...
- [x] Dependency direction reviewed: local dependencies are tdw-cdc, tdw-protocol, tdw-rollout; reverse dependencies are tdw-service-api.
...
- Replay engine builds dry-run plans from CDC offsets and protocol replay plans from rollout records.
- No code change required in G002; existing tests cover CDC dry-run and rollout event-type extraction.
- Follow-up boundary: Mutation replay execution and authorization gates belong to runtime/client tranches.
