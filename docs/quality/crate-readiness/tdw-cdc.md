...
- [x] Dependency direction reviewed: local dependencies are tdw-event, tdw-outbox; reverse dependencies are tdw-replay, tdw-service-api.
...
- CDC records preserve outbox sequence, event id, event type, and payload and expose after-offset reads.
- No code change required in G002; existing tests cover ordered conversion from pending outbox records.
- Follow-up boundary: Persistent CDC checkpoint storage belongs to storage/runtime tranches.
