# tdw-protocol Readiness Worksheet

Owner tranche: G002-core-contracts-event-session-and-replay-crates - Core Contracts, Event, Session, and Replay Crates.

## Baseline Inventory

- Manifest: crates\tdw-protocol\Cargo.toml
- Target kinds: lib
- Local dependencies: none
- External dependencies: schemars ^1.2.1; serde ^1.0.228 features=[derive]; serde_json ^1.0.145; thiserror ^2.0.18; ulid ^1.2.1 features=[serde]; uuid ^1.23.1 features=[v7,serde]
- Dev dependencies: none
- Reverse local dependencies: tdw-acp, tdw-app-client, tdw-app-server, tdw-exec, tdw-hooks, tdw-replay, tdw-rollout, tdw-service-api, tdw-session, tdw-tools, tdw-tui, xtask
- Feature flags: none
...
- [x] Dependency direction reviewed: local dependencies are none; reverse dependencies are tdw-acp, tdw-app-client, tdw-app-server, tdw-exec, tdw-hooks, tdw-replay, tdw-rollout, tdw-service-api, tdw-session, tdw-tools, tdw-tui, xtask.
...
- Session, operation, permission, tool-call, op, event, and replay-frame contracts are typed and schema-exported.
- No code change required in G002; non-empty ID validation and replay serialization tests cover the current protocol boundary.
- Follow-up boundary: Future protocol expansion must preserve schema fixture compatibility.
